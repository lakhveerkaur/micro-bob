...
module.exports = function(io, socket) {
    ...
    function handleFindToken(userName){
        GoogleAToken.findOne({username:userName},function(err,reply){
            if(reply == null){
                console.log("PREF IN SOCKET",reply);
                UserInfo.find({username:userName},function(err,reply){

                    socket.emit('tokenNotFound',reply[0].preferences);
                });

           }
            else{
                console.log('Token found',reply);
            }
        });
    }
    ...
    function handleSubscribeMe(channelName) {
        ...
        var projectname = channelName.split('#')[0];
        sub.subscribe(channelName);
        console.log(projectname,"project newly added");
        //sub.subscribe(projectname);
        pub.hgetall(projectname+":s3",function(err,result){
            console.log(result,"yeah got members==========");
            socket.emit("newProjectStatus",result,channelName);
        })
            ...
            .then(function(res) {
                ...
                if(res.entities.length>0){
                    for(let i=0;i<res.entities.length;i++)
                    {
                        ...
                        else if(res.entities[i].name === "datetime"){
                            entity = {"datetime" : res.entities[i].formatted}
                            entities.push(entity)
                        }
                        else if(res.entities[i].name === "duration"){
                            entity = {"duration" : res.entities[i].raw}
                            entities.push(entity)
                        }
                        else{
                            console.log()
                    ...
                    if(flag === 1){
                        ...
                        socket.on('reminderAccepted',function(username, summary, location, startDate, endDate){
                            var reminderObj = {'summary':summary,'location':location,'startDate':startDate,'endDate':endDate};  
    ...
    function handleLogin(socket, usrname) {
        ...
        function(err){
           ...
           // socket.broadcast.emit('getStatus',getProject);
    ...
    console.log(membersList,"member----------")
    let TeamName = [];
    let projectStatus =[];
    ...
    for(key in result){
        ...
        if(key == member){
            ...
            updateUserStatusforProject(projectName,key,result[key],callback);
           //break;
        }
    }
    },function(err){
   ...
   // console.log(arr,"new array");
            ...
            //     for(key in result){
            //     if(key == Projectmembers[i]){
            //         obj[key] = result[key];
            //         onUserstatusChangeforProject(projectName,key,result[key]);
            //         break;
            //     }
        ...
        // users:obj
        // }
        // if(projectStatus[0] == null){
        //     projectStatus.push(obj2);
        //     console.log(projectStatus,"project with status");
            
        //     // socket.emit('getStatus1',projectStatus);
        //    // for(var key in obj2)
        //     // console.log(key,"key");
        //     // console.log(obj2[key],"status");
        //    // pub.hmset(projectName,obj2[key]);
            
        // }
        // for(var key in obj2.users){
        //       //console.log(key,obj2.users[key],"status");
        //     //  pub.hmset(projectName+":s3",key,obj2.users[key]);
        //     }
        //console.log(projectStatus,"object+++++++++");   
    ...
    //    }
    //    else{
    //         console.log(projectStatus,"object________");
    //    }
    // })
